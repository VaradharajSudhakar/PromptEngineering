...
def summarize_output(output, query):
    ...
    summary_response = openai_llm.invoke(prompt)
    summary = summary_response.content
    return summary

# Streamlit app setup
st.title("AI SQL Agent")
st.subheader("Accenture : GenAI Learning Team")
...
if st.button("Submit"):
    ...
    if user_query:
        ...
        Microsoft SQL Server and the table name is houses(id, property, location, price, dive) ex (4, 'Mountain cabin', 'Aspen, Colorado', 850000, 'Rustic'),
           into a SQL query and if it has limit replace it with top :\n'{user_query}' and return only the sql query with a semi-colon at the end of each sql query."""
        llm_response = openai_llm.invoke(prompt)
